...
import { MappingStore } from "./GumTree";
...
export class ZsMatcher {
    ...
    constructor(srcTree: BragiAST, dstTree: BragiAST, mappings: MappingStore, srcMetrics: TreeMetricComputer, dstMetrics: TreeMetricComputer) {
    ...
    match() {
        ...
        while (treePairs.length > 0) {
            ...
            if (!rootNodePair)
                this.forestDistFn(lastRow, lastCol);
            ...
            while (row > firstRow || col > firstCol) {
                if (row > firstRow && this.forestDist[row - 1][col] + 1 === this.forestDist[row][col]) {
                    ...
                    if (this.zsSrc.lld(row) - 1 === this.zsSrc.lld(lastRow) - 1
                        ...
                        if (tSrc.type === tDst.type)
                            this.mappings.addMapping(tSrc.id, tDst.id);
                        row--;
                        col--;
                    } else {
    ...
    private computeTreeDist() {
        ...
        for (let i = 1; i < this.zsSrc.kr.length; i++)
            for (let j = 1; j < this.zsDst.kr.length; j++)
                this.forestDistFn(this.zsSrc.kr[i], this.zsDst.kr[j]);
    }
