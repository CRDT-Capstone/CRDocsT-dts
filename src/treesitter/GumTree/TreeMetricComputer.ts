import { allChildIds, AstNode, BragiAST, NodeId } from "../types/AST.js";
import { TreeMetrics } from "./TreeMetrics.js";
...
export class TreeMetricComputer {
    ...
    startNode(tree: BragiAST, node: AstNode) {
        const children = allChildIds(tree, node);
        if (children.length === 0) this.visitLeaf(node);
        else this.startInnerNode(node);
    }

    endTree(tree: BragiAST, node: AstNode) {
        const children = allChildIds(tree, node);
        if (children.length > 0) this.endInnerNode(tree, node);
    }
    ...
    visitLeaf(node: AstNode) {
        this.metrics.set(
            ...
            new TreeMetrics(
                1,
                0,
                TreeMetricComputer.leafHash(node),
                TreeMetricComputer.leafStructureHash(node),
                this.currentDepth,
    ...
    private static innerNodeHash(node: AstNode, size: number, middleHash: number): number {
        return (
            parseInt(hash({ type: node.type, text: node.text, action: TreeMetricComputer.ENTER }), 16) +
            middleHash +
    ...
    private static innerNodeStructureHash(node: AstNode, size: number, middleHash: number) {
        return (
            parseInt(hash({ type: node.type, action: TreeMetricComputer.ENTER }), 16) +
            middleHash +
            parseInt(hash({ type: node.type, action: TreeMetricComputer.LEAVE }), 16) *
                TreeMetricComputer.hashFactor(size)
        );
    }
