...
import { pqType } from "../GumTreeTopDown.js";
...
export class HashBasedMapper {
    ...
    ambiguous() {
        return Array.from(this.mapping.values()).filter(
            (pair) => (pair.first.size > 1 && pair.second.size >= 1) || (pair.first.size >= 1 && pair.second.size > 1),
        );
    }
