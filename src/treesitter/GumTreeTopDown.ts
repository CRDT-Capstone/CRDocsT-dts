...
export class GumTreeTopDown {
    ...
    private AddChildrenToQueue(
        tree: BragiAST,
        node: AstNode,
        PQ: FastPriorityQueue<pqType>,
        metricComputer: TreeMetricComputer,
    ) {
    ...
    topDown = () => {
        ...
        while (this.synchronize()) {
            ...
            localHashMappings.ambiguous().forEach((pair) => {
                ambiguousMappings.push(pair);
            });
    ...
    handleAmbiguousMappings(ambiguousMappings: Pair<Set<AstNode>>[]) {
        const comparator: FullMappingComparator = new FullMappingComparator(
            this.mappings,
            this.srcTree,
            this.dstTree,
            this.srcMetrics,
    ...
    private synchronize() {
        while (
            !this.srcTreePQ.isEmpty() &&
            !this.dstTreePQ.isEmpty() &&
