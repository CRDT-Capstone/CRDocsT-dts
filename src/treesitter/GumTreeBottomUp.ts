...
import { diceSimilarity, getDescendants, getParent } from "./types/GumTree/utils";
import { postorderAstTraversal } from "./utils";
import { TreeMetricComputer } from "./types/GumTree/TreeMetricComputer";
import { TreeMetrics } from "./types/GumTree/TreeMetrics";
import { ZsMatcher } from "./types/GumTree/ZsMatcher";
...
export class GumTreeBottomUp {
    ...
    constructor(srcTree: BragiAST, dstTree: BragiAST, srcMetricsComputer: TreeMetricComputer, dstMetricsComputer: TreeMetricComputer) {
    ...
    match(srcNode: AstNode, dstNode: AstNode, mappings: MappingStore) {
        ...
        postorderAstTraversal(this.srcTree, (node) => { postOrder.push(node) });
...
lastChanceMatch(mappings: MappingStore, srcNode: AstNode, dstNode: AstNode) {
    if (this.srcMetrics.get(srcNode.id)!.size < this.sizeThreshold
        ...
        for (const mapping of zsMappings) {
            if (mappings.isMappingAllowed(mapping.f, mapping.s))
                mappings.addMapping(mapping.f, mapping.s);
        }
    }
}
}